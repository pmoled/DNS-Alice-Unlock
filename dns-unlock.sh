...
# 提示用户是否调整配置文件中的 IP
    read -p "配置文件中 IP 为 166.0.199.207 和 166.0.199.207，是否调整？(回车默认Alice DNS，输入y调整自己的解锁IP): " adjust
    if [[ "$adjust" == "y" || "$adjust" == "Y" ]]; then
        read -p "请输入您的解锁IP: " unlock_ip
        # 修改配置文件中的 IP 地址
        echo -e "\033[1;34m正在修改配置文件中的 IP 地址...\033[0m"
        sed -i "s/166.0.199.207/$unlock_ip/g" $CONFIG_FILE
        sed -i "s/166.0.199.207/$unlock_ip/g" $CONFIG_FILE
...
3)
  ...
  1)
    # 更新为 HK 配置
    CONFIG_URL="https://raw.githubusercontent.com/pmoled/DNS-Alice-Unlock/refs/heads/main/dnsmasq.conf.hk"
    TARGET_FILE="dnsmasq.conf.hk"
    REGION="HK"
    ;;
  2)
    # 更新为 SG 配置
    CONFIG_URL="https://raw.githubusercontent.com/pmoled/DNS-Alice-Unlock/refs/heads/main/dnsmasq.conf.sg"
    TARGET_FILE="dnsmasq.conf.sg"
    REGION="SG"
    ;;
  3)
    # 更新为全量配置
    CONFIG_URL="https://raw.githubusercontent.com/pmoled/DNS-Alice-Unlock/refs/heads/main/dnsmasq.conf.allsg"
  ...
  *)
  ...
  if [ $? -eq 0 ]; then
    ...
    if [ "$REGION" == "AllSG" ]; then
      echo -e "\033[1;33m配置文件中包含 IP 地址 166.0.199.207，是否需要替换为自己的 IP 地址？(回车默认不修改，输入 y 修改)\033[0m"
      read use_own_ip
      ...
      if [ "$use_own_ip" == "y" ]; then
        ...
        read user_ip
        # 替换文件中的 IP 地址
        sed -i "s/166.0.199.207/$user_ip/g" /etc/$TARGET_FILE
...
curl -o /etc/smartdns/smartdns.conf https://raw.githubusercontent.com/pmoled/DNS-Alice-Unlock/refs/heads/main/smartdns.conf
if [ $? -ne 0 ]; then
...
DEFAULT_IP1="166.0.199.207"
DEFAULT_IP2="166.0.199.207"
...
;;
 ...
 3)
    ...
    4)
      # 一键更新全量配置
      CONFIG_URL="https://raw.githubusercontent.com/pmoled/DNS-Alice-Unlock/refs/heads/main/smartdns.conf.sg"
      ...
      echo "检测到配置文件中可能需要更换的 IP：166.0.199.207"
      ...
      if [[ "$replace_choice" =~ ^[Yy]$ ]]; then
        ...
        echo -e "\033[1;32m已将 166.0.199.207 替换为 $new_ip\033[0m"
      fi
...
7)
  ...
  # 获取远程脚本的版本号
  REMOTE_VERSION=$(curl -s https://raw.githubusercontent.com/pmoled/DNS-Alice-Unlock/refs/heads/main/dns-unlock.sh | grep "VERSION=" | cut -d '"' -f 2)
 ...
 CURRENT_VERSION=$(grep 'VERSION=' /root/dns-unlock.sh | cut -d '"' -f 2)
  ...
  if [ "$REMOTE_VERSION" \> "$CURRENT_VERSION" ]; then
    ...
    # 下载并替换当前脚本
    curl -o /root/dns-unlock.sh https://raw.githubusercontent.com/pmoled/DNS-Alice-Unlock/refs/heads/main/dns-unlock.sh
    if [ $? -eq 0 ]; then
